...
echo "3. Docker Compose mode (Detached with compose file): Uses Password File"
...
if [ "$PASSWORD_OPTION" -eq 1 ] || [ "$PASSWORD_OPTION" -eq 3 ]; then
    ...
    if [ "$PASSWORD_OPTION" -eq 3 ]; then
        echo "Docker Compose mode Setup"
    else
...
services:
  $CONTAINER_NAME:
    ...
    volumes:
      - $CONFIG_PATH:/keys
      - $PASSWORD_FILE:/run/secrets/keystore_password:ro
    command: >
      start
...
if [[ ${CREATE_SCRIPT:0:1} =~ ^[Yy]$ ]]; then
    ...
    if $CREATE_FILE; then
        if [ "$PASSWORD_OPTION" -eq 3 ]; then
            ...
            cat > "$SCRIPT_PATH" << EOF
#!/bin/bash
# Ejector start script - Generated by installation script - Docker Compose Mode
...
echo -e "\033[1;32m✓ Container started successfully (using Docker Compose)\033[0m"
...
EOF
        ...
        if [ "$PASSWORD_OPTION" -eq 3 ]; then
            echo -e "\033[1;32m✓\033[0m Docker Compose start script created successfully"
        elif [ "$PASSWORD_OPTION" -eq 1 ]; then
...
if [ "$PASSWORD_OPTION" -eq 3 ]; then
    echo -e "    Startup Method: \033[1;36mDocker Compose\033[0m"
...
if [[ ${START_CLIENT:0:1} =~ ^[Yy]$ ]]; then
    if [ "$PASSWORD_OPTION" -eq 3 ]; then
        ...
        echo -e "\033[1;32m✓ Service started successfully using Docker Compose\033[0m"
